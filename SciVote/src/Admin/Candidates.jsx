...
export default function Candidates({ onBack }) {
  ...
  const addPartyList = async () => {
    ...
    if (
      partyLists.some(
        (party) => party.toLowerCase() === newPartyName.trim().toLowerCase()
      )
    ) {
    ...
    try {
      ...
      setSuccessMessage(
        `"${newPartyName.trim()}" party list added successfully!`
      );
  ...
  const removePartyList = async (partyToRemove) => {
    ...
    const candidatesUsingParty = candidates.filter(
      (c) => c.partyList === partyToRemove
    );
    if (candidatesUsingParty.length > 0) {
      setErrorMessage(
        `Cannot remove "${partyToRemove}" because ${candidatesUsingParty.length} candidate(s) are assigned to this party.`
      );
      return;
    }
  ...
  const getStudentPhoto = (schoolId) => {
    if (!schoolId || !registeredStudents.length) return null;
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-600">
            Add and manage candidates for the election
          </p>
        </div>
        <button
          onClick={onBack}
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-yellow-700">
                <p>
                  You need to register students first before you can add
                  candidates. Please go to the Student Registration section to
                  add students.
                </p>
              </div>
            </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gradient-to-r from-slate-100 to-gray-100">
              <th className="text-left p-4 font-semibold text-slate-700 rounded-tl-xl">
                Photo
              </th>
              ...
              <th className="text-center p-4 font-semibold text-slate-700 rounded-tr-xl">
                Actions
              </th>
            </tr>
          </thead>
